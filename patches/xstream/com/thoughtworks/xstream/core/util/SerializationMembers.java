/*
 * XStream 1.4.20 SerializationMembers with a defensive fallback for CheerpJ.
 *
 * Based on com.thoughtworks.xstream.core.util.SerializationMembers from
 * XStream 1.4.20 (BSD-style license, Copyright (C) 2004-2021 XStream
 * Committers / Joe Walnes / Joerg Schaible). Modified for CheerpJ 4.3,
 * which can throw ArrayIndexOutOfBoundsException from reflective lookups
 * inside callReadResolve/callWriteReplace in specific GUI contexts
 * (observed while Digital deserializes ROMManagerFile on the AWT EDT).
 * Digital's persisted classes do not implement readResolve/writeReplace,
 * so degrading to "no such method" is safe and strictly better than
...
public class SerializationMembers implements Caching {
    ...
    public Object callReadResolve(final Object result) {
        ...
        try {
            return callReadResolveInner(result);
        } catch (final ArrayIndexOutOfBoundsException e) {
            // CheerpJ runtime bug — no readResolve support in this context.
            System.out.println("[xstream-patch] callReadResolve degraded for "
                + result.getClass().getName() + ": " + e);
            return result;
        }
    }

    private Object callReadResolveInner(final Object result) {
    ...
    public Object callWriteReplace(final Object object) {
        ...
        try {
            return callWriteReplaceInner(object);
        } catch (final ArrayIndexOutOfBoundsException e) {
            System.out.println("[xstream-patch] callWriteReplace degraded for "
                + object.getClass().getName() + ": " + e);
            return object;
        }
    }

    private Object callWriteReplaceInner(final Object object) {
